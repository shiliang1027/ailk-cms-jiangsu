package com.linkage.module.cms.oneline.serv.wireless
{
	 ...
	 *
	 */
	public class NormalCellGisInfoDataCMD
	{
		private var log:ILogger=Log.getLoggerByClass(GisHotHistoryGisInfoDataCMD);
		...
		{
			log.debug("[查询普通小区GIS点信息数据]");
			if(!e.param){
				e.param = new Object();
			}
			e.param.areaId=user.areaId;
			log.debug(e.param);
			return dao.getNormalCellGisInfo(e.param);
		}
		...
		{
			log.debug("[查询普通小区GIS点信息数据结果]");
			log.debug(obj);
			dm.gisInfos.removeAll();
			dm.gisInfos.addAll(ArrayCollection(obj));
			log.info("dm.gisInfos--------------"+ dm.gisInfos.length);
			log.info(dm.gisInfos);
			dm.queryGisInfoFlag=false;//设置查询实时热点标志为false,2013-9-3,汪炜;
			dispatchMsg(new GisHotEvent(GisHotEvent.LOADNORMALCELLGISINFODATACOMPLETE));
		}
		
		public function error(f:Fault):void
		{
			log.error("[查询普通小区GIS点信息数据]失败：" + f.faultString + "::" +f.faultDetail);
