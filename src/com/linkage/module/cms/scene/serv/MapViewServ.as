package com.linkage.module.cms.scene.serv
{
    ...
    public class MapViewServ
    {
        ...
        public function dealSnapshot(e:SceneEvent):void
        {
            ...
            if (user.initSceneId == "1004" && user.level == "2")
                ;
            else
                for each (var imgInfo:Object in mapViewDM.snapshotList)
                {
                    ...
                    log.debug("pic.width:" + pic.width + "   pic.height:" + pic.height);
...
//                    var off:Object = jingquOffset[imgInfo['areaName']];
        ...
        public function dealPolygon(e:SceneEvent):void
        {
            ...
            {
                if (user.initSceneId != "1011" && user.initSceneId != "1003" && user.initSceneId != "1006")
                {
                ...
                if (moInfo['moTypeId'] == '16' || moInfo['moTypeId'] == '28' || moInfo['moTypeId'] == '320')
                {
                    ...
                    for (var i:int = 0; i < moInfo['cells'].length && i < 4; i++)
                    {
                        ...
                        {
                            if (mapViewDM.emgCarMap.indexOf(moInfo["moId"]) != -1)
                                pic = new GisMarkerPictureStyle(ImageFactory.getImage('sector', temp["warnLevel"]), 15, 20, sectorOffSet3[i]['offx'], sectorOffSet3[i]['offy'], 1, angle);
                            else
                        ...
                        {
                            if (mapViewDM.emgCarMap.indexOf(moInfo["moId"]) != -1)
                                pic = new GisMarkerPictureStyle(ImageFactory.getImage('sector', temp["warnLevel"]), 15, 20, sectorOffSet4[i]['offx'], sectorOffSet4[i]['offy'], 1, angle);
                            else
            ...
            log.warn("POLYGON处理耗时：{0}", (new Date).getTime() - systime);
			log.warn("[POLYGON处理]: 结束：{0}", new Date().toString());
        }

        private var firstPolygon:Boolean = true;
        ...
        private function changeViewByGisImg(gisFeature:GisFeature, event:MouseEvent):void
        {
            ...
            user.sceneId = gisFeature.attributes.areaId;
            user.layerurl = gisFeature.attributes.layerurl;
            user.layerId = gisFeature.attributes.layerId;
            user.level = "2";

            var evt:SceneEvent = new SceneEvent(SceneEvent.CHANGE_MAPVIEW);//TODO
