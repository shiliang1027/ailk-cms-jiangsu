package com.linkage.module.cms.scene.serv
{
	 ...
	 *
	 */
	public class ShareSceneListCmd
	{
		private var log:ILogger = Log.getLoggerByClass(ShareSceneListCmd);
		...
		{
			log.debug("[共享.共享场景：]cmd命令捕获execute开始执行...");
			return scenePanelDao.queryShareScene(e.param);
		}

		public function result(data:Object):void
		{
			log.debug("[共享.共享场景：]result info ...");
			log.debug(data);
		...
		{
			log.error("[共享.共享场景：]cmd命令返回失败error..." + f.faultString + "\n" + f.faultDetail);
			msgDispatcher(new Event("REQUEST_COMPLETE"));
		}
	}
}
