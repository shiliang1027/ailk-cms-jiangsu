package com.linkage.module.cms.baseline.dynamic.data
{
	...
	public class BaseLineDataManagerBS implements BaseLineDataManager
	{
		...
		private static var log:ILogger = Log.getLogger("com.linkage.module.cms.baseline.dynamic.data.BaseLineDataManagerBS");
		 ...
		 * 保存基线算法类型
		 **/ 
		public function saveBSLineEvaltype(param:Object,success:Function,complete:Function=null,error:Function=null):void{
			var remoteService:RemoteObject=BlazeDSUtil.newService(DESTINATION, SOURCE, endpoint, function(result:Object):void
			{
				log.info("-----saveBSLineEvaltype :成功：" + (result == null? "0" : result.length));
				success.call(null, result);
			}, function(event:FaultEvent):void
			{
				log.error("-----saveBSLineEvaltype :失败：" + event.fault.faultString);
				error.call(null, event);
			},true,"saveBSLineEvaltype");
			log.info("-----saveBSLineEvaltype ,参数： param:{0}", param);
			remoteService.saveBSLineEvaltype(param);
			remoteService=null;
		}
		 ...
		 * 删除基线
		 **/ 
		public function deleteBaseLine(param:Object,success:Function,complete:Function=null,error:Function=null):void{
			var remoteService:RemoteObject=BlazeDSUtil.newService(DESTINATION, SOURCE, endpoint, function(result:Object):void
			{
				log.info("-----removeBaseLine :成功：" + (result == null? "0" : result.length));
				success.call(null, result);
			}, function(event:FaultEvent):void
			{
				log.error("-----removeBaseLine :失败：" + event.fault.faultString);
				error.call(null, event);
			},true,"removeBaseLine");
			
			log.info("-----removeBaseLine ,参数： param:{0}", param);
			remoteService.removeBaseLine(param);
			remoteService=null;
		}
