package com.linkage.module.cms.groupclientalarm.map
{
	...
	public class MapWork extends Group
	{
		...
		private var bgMV:MovieClip;
		private var bg:UIComponent = new UIComponent();



		private var _mapNodeEvent:MapNodeEvent;

		public function setBackGround(bgMovieClip:MovieClip):void
		{
			this.bgMV = bgMovieClip;
			bg.addChild(bgMV);
		}
...
//				mapNodeB.mouseOver();
				...
				if (_mapNode && _mapNode.id != mapNodeA.id)
				{
...
//					mapNodeBB.mouseOver();
					...
					this.dispatchEvent(_mapNodeEvent);
					if (_mapNode && _mapNode.id != mapNodeAA.id)
					{
						areaId = getAreaId(_mapNode.id);
...
//					mapLabel.styleName = "mapLabel";
					//暂时修改为黑色，加粗样式。
					mapLabel.setStyle("color",0x000000);
					mapLabel.setStyle("font-weight","bold");
