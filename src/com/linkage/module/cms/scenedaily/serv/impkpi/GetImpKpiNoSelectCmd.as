package com.linkage.module.cms.scenedaily.serv.impkpi
{
	 ...
	 *
	 */
	public class GetImpKpiNoSelectCmd
	{
		private var log:ILogger = Log.getLoggerByClass(GetImpKpiNoSelectCmd);
		[Inject("impProKpiDM")]
		...
		public function result(data:Object):void
		{
			...
			log.debug(data);
			impProKpiDM.kpiResultNoSelect.removeAll();
			impProKpiDM.kpiResultNoSelect.addAll(data as IList);
			impProKpiDM.kpiResultNoSelect.refresh();
			log.debug(impProKpiDM.kpiResultNoSelect);
		}

		public function error(f:Fault):void
		{
			log.error("[重点指标性能]:未选择列表cmd返回失败..." + f.faultString + "," + f.faultDetail);
		}
	}
}