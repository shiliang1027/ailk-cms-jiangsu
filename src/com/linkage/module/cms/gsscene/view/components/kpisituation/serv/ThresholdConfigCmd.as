package com.linkage.module.cms.gsscene.view.components.kpisituation.serv
{
	 ...
	 *
	 */
	public class ThresholdConfigCmd
	{
		private var log:ILogger = Log.getLoggerByClass(ThresholdConfigCmd);
		...
		{
			log.debug("[阈值保存]:cmd命令捕获execute开始执行...");
			return scenePanelDao.thresholdConfig(e.param);
		}

		public function result(data:Object):void
		{
			log.debug("[阈值保存]:result info ...");
			log.debug(data);
			kpiSituationDM.kpiConfigSaveResult = data as String;
			log.debug(kpiSituationDM.kpiConfigSaveResult);
			var e:KPISituationEvent = new KPISituationEvent(KPISituationEvent.CMS_SCENE_KPISITUATION_CONFIG_SAVE_RESULT);
		...
		{
			log.error("[阈值保存]:cmd命令返回失败error..." + f.faultString + "\n" + f.faultDetail);
