...
//			return rmtObj.getSceneEmgyDetail({'empergResType': param.emergrestype, "isUsed": param.isUsed, "sceneId": user.sceneId, "level": user.level, "cityId": user.cityId});
			return rmtObj.getSceneEmgyDetail(param);
		}

		/**
		 * 查询应急资源轨迹
		 */
		public function getSceneEmgyChg(param:Object):AsyncToken
		{
			init(param);
			log.debug("查询应急资源轨迹");
			return rmtObj.getSceneEmgyChg(param);
		}
		/**
		 * 获取重点指标数据
		 */
		...
		public function getSceneWeatherInfo():AsyncToken
		{
			...
			init();
			return rmtObj.getSceneWeatherInfo(user);
		}
		...
		public function queryBtsAndQuitBtsNum():AsyncToken
		{
			init();
			return rmtObj.queryBtsAndQuitBtsNum(user);
		}
